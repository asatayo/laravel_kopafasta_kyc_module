              ...
              <h6>SOMA KWA MAKINI MAELEKEZO YANAYOHUSU UJAZAJI WA FOMU HII YALIYOAMBATANISHWA</h6>
                <div class="row">
                    <div class="col-md-12 mx-0">
                        <form id="msform" class="form" method="POST" action="{{route('ndingaLoanTwo')}}">
                          @csrf
                          <input type="hidden" name="ndinga" value="{{$ndingaLoan->id}}">
                                       ...
                                       <div class="col-md-4">
                                         <div class="form-group">
                                           <label for="debt_amount" class="text-black">Kiasi</label>
                                             <input type="text" onkeyup="formatNumber('debt_amount')"  class="form-control form-control-lg debt_amount" name="debt_amount" placeholder="Kiasi"/>
                            ...
                            <div class="row mt-5">
                              <table class="table">
                                 ...
                                 <tbody>
                                   @foreach($ndingaOtherDebts as $key => $ndingaOtherDebt)
                                   <tr>
                                     <th scope="row">{{ $key+1 }}</th>
                                     <td>{{$ndingaOtherDebt->amount}}</td>
                                     <td>{{$ndingaOtherDebt->debt_institution}}</td>
                                     <td>{{ Carbon\Carbon::parse($ndingaOtherDebt->finish_date)->format('Y-m-d')}}</td>
                                     <td> <a class="btn btn-danger" href="{{url('kycmodule/loan/ndinga/debt-delete', [$ndingaLoan->id,$ndingaOtherDebt->id])}}">Delete</a> </td>
                            ...
                            <a href="{{ url('kycmodule/loan/ndinga',[$ndingaLoan->id, 'step-three']) }}"  class="float-end btn btn-success me-md-5"/> Hakiki na Hifadhi <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></a>
