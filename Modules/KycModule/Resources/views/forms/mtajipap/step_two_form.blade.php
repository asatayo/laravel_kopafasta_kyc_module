              ...
              <h6>SOMA KWA MAKINI MAELEKEZO YANAYOHUSU UJAZAJI WA FOMU HII YALIYOAMBATANISHWA</h6>
                <div class="row">
                    <div class="col-md-12 mx-0">
                        <form id="msform" class="form" method="POST" action="{{route('mtajipapLoanTwo')}}">
                          @csrf
                          <input type="hidden" name="mtajipap" value="{{$mtajipapLoan->id}}">
                            ...
                            <div class="row mt-5">
                              <table class="table">
                                 ...
                                 <tbody>
                                   @foreach($mtajipapOtherDebts as $key => $mtajipapOtherDebt)
                                   <tr>
                                     <th scope="row">{{ $key+1 }}</th>
                                     <td>{{$mtajipapOtherDebt->amount}}</td>
                                     <td>{{$mtajipapOtherDebt->debt_institution}}</td>
                                     <td>{{ Carbon\Carbon::parse($mtajipapOtherDebt->finish_date)->format('Y-m-d')}}</td>
                                     <td> <a class="btn btn-danger" href="{{url('kycmodule/loan/mtajipap/debt-delete', [$mtajipapLoan->id,$mtajipapOtherDebt->id])}}">Delete</a> </td>
                            ...
                            <a href="{{ url('kycmodule/loan/mtajipap',[$mtajipapLoan->id, 'step-three']) }}"  class="float-end btn btn-success me-md-5"/> Hakiki na Hifadhi <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></a>
